...
def circlize_etl(sixteenS_tax, eighteenS_tax, graph_path, focus_group, top_n):
    sixteenS = sixteenS_tax.copy(deep=True)
    eighteenS = eighteenS_tax.copy(deep=True)
    ...
    taxonDict = {}
    for node, taxon in sixteenS['Taxonomy'].items():
        taxonDict[node] = {'Taxon': taxon}
    for node, taxon in eighteenS['Taxonomy'].items():
        taxonDict[node] = {'Taxon': taxon}
    ...
    # Get edgelist
    edgelist = get_edgelist(sixteenS_tax, eighteenS_tax, graph_path)
    # Get diatom edges
...
def get_edgelist(sixteenS_tax, eighteenS_tax, graph_path):
    """
    Generate edgelist node1, node2, node1Taxonomy, node2Taxonomy, weight from sixteens, eighteens, graph files
    """
    # Combined 16S and 18S taxonomies into one column files
    sixteenS = sixteenS_tax.copy(deep=True)
    eighteenS = eighteenS_tax.copy(deep=True)
    sixteenS.index = pd.Series(sixteenS.index).apply(lambda x: 'potu_' + str(x))
    eighteenS.index = pd.Series(eighteenS.index).apply(lambda x: 'eotu_' + str(x))
    ...
    for node, taxon in sixteenS['Taxonomy'].items():
        taxonDict[node] = {'Taxon': taxon}
    for node, otuid in sixteenS['OTUID'].items():
        otuDict[node] = {'OTUID': otuid}
    for node, taxon in eighteenS['Taxonomy'].items():
        taxonDict[node] = {'Taxon': taxon}
    for node, otuid in eighteenS['OTUID'].items():
        otuDict[node] = {'OTUID': otuid}
    nx.set_node_attributes(gph, taxonDict)
    nx.set_node_attributes(gph, otuDict)
    # build edgelist
    nodetaxons = nx.get_node_attributes(gph, 'Taxon')
    nodeids = nx.get_node_attributes(gph, 'OTUID')
    edgelist = nx.to_pandas_edgelist(gph, source='node1', target='node2')
    edgelist['node1OTUID'] = edgelist['node1'].apply(lambda x: nodeids[x] if x in nodeids else np.nan)
    edgelist['node2OTUID'] = edgelist['node2'].apply(lambda x: nodeids[x] if x in nodeids else np.nan)
    ...
    edgelist = edgelist[['node1', 'node2', 'node1OTUID', 'node2OTUID', 'node1Taxonomy', 'node2Taxonomy', 'weight']]
    return edgelist
